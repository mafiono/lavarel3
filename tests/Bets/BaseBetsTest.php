...
abstract class BaseBetsTest extends TestCase
{
    protected $user;
    ...
    protected function makeBetForUser($userId, $amount, $odds = 1.5, $modifiers = [], $events = [])
    {
        $eventsCount = $events;
        if (is_array($events)) {
        ...
        if (is_array($events)) {
            ...
            foreach ($events as $event) {
                $eventsObj[] = factory(App\Bets\Bets\Events\BetslipEvent::class)->make(
                    $this->applyModifiers([], App\UserBetEvent::class,
                        [App\UserBetEvent::class => $event])
                );
            }
            $eventsObj = new Collection($eventsObj);
        } else {
            $eventsObj = factory(App\Bets\Bets\Events\BetslipEvent::class, $eventsCount)->make(
                $this->applyModifiers([], App\UserBetEvent::class, $modifiers)
            );
        }

        if (!is_a($eventsObj, Collection::class)) {
            $eventsObj = collect([$eventsObj]);
        }

        $eventsObj->each(function ($event) use ($bet) {
    ...
    protected function placeBetForUser($userId, $amount, $odds = 1.5, $modifiers = [], $events = 1)
    {
        $bet = $this->makeBetForUser($userId, $amount, $odds, $modifiers, $events);
    ...
    protected function resolveBet(Bet $bet, $status = 'won')
    {
        $betResolver = new \App\Bets\Resolvers\BetResolver();
        $reflection = new ReflectionClass($betResolver);
        $method = $reflection->getMethod('resolveBet');
        $method->setAccessible(true);
        $method->invoke($betResolver, $bet, $status);
    }
